...
def main():
    ...
    p.add_argument("-b", "--baud", default=115200, help="baud rate for connection")

    # deploy command
    p = subparsers.add_parser("deploy", help="deploy project to device")
    p.set_defaults(cmd=commands.deploy_command)
    p.add_argument(
        "-d", "--device", default=None, help="path of device to deploy to project to"
    )
    p.add_argument(
        "-y",
        "--yes",
        action="store_true",
        help="deploy to discovered device without confirmation dialog",
    )
    ...
    p.add_argument(
        "-m", "--modules", action="store_true", help="list availiable modules"
    )
