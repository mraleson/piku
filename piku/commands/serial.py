...
def serial_command(args):
    ...
    print(
        f'Connecting to {"DEFAULT" if using_default else "SPECIFIED"} serial device {port} using baud {baud}...'
    )
    ...
    if not port:
        print(
            "Failed to find the specified serial port or find a default serial port to use."
        )
        return
