...
def has_correct_label(path):
    if platform.system() == "Windows":
        drive = path.split(":")[0]
        label = (
            check_output(f"cmd /c vol {drive}:".split())
            .decode()
            .split("\r\n")[0]
            .split(" ")
            .pop()
...
def find_device_path():
    if platform.system() == "Windows":
        output = check_output(
            "wmic logicaldisk where drivetype=2 get DeviceId , VolumeName".split()
        ).decode()
        drives = [
            line.split(" ")[0]
...
def deploy_command(args):
    ...
    if not has_correct_size(device):
        print(
            "Refusing to deploy, specified CircuitPython drive is larger than expected (~2MB)."
        )
        return
    ...
    if not args.yes:
        ...
        print(
            "WARNING THIS WILL REMOVE ALL OTHER FILES FROM THE DEVICE! PLEASE BE CAREFUL!"
        )
